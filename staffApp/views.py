...
def staff_view(request):
    if request.method == 'GET':
        ...
        if stff_id and staff_username:
            # 往数据库添加数据
            staff = Teacher.objects.create(tea_id=stff_id, name=staff_username, gender=staff_gender, age=staff_age,
                                           address=staff_address, tel_num=staff_phone, email=staff_email,
                                           birth=staff_birth, education=staff_eb, over_school=staff_college,
                                           id_num=staff_id, ms=staff_marry, create_date=staff_date,
                                           leave_date=staff_leave_date, is_delete=staff_dimission)
...
def staffupdated_view(request, pid):
    ...
    staff_date = request.POST.get('staff_date', '')
    staff_leave_date = request.POST.get('staff_leave_date', '')
    staffs = Teacher.objects.filter(tea_id=pid).update(tea_id=stff_id, name=staff_username, age=staff_age,
                                                       address=staff_address, tel_num=staff_phone, email=staff_email,
                                                       birth=staff_birth, over_school=staff_college, id_num=staff_id,
                                                       create_date=staff_date, leave_date=staff_leave_date)
